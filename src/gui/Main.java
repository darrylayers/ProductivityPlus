...
public class Main {
	...
	public Main() {
		...
		frame.getContentPane().setLayout(new MigLayout("", "[1061.00px]", "[482.00px]"));
		mainPanel.setBackground(Color.WHITE);
		mainPanel.setLayout(new MigLayout("", "[110.00][224.00,grow]", "[35.00][414.00,grow]"));
		...
		trackPanel.setLayout(new MigLayout("", "[310.00px,grow][450,grow][501.00,grow]", "[75][457.00px,grow]"));
		...
		buttonPanel.setLayout(new MigLayout("", "[100,grow]", "[25][][][][][][][][38.00,grow][][][][][]"));
		...
		JCheckBox chckbxConsolidatePrograms = new JCheckBox("Consolidate Programs");
		chckbxConsolidatePrograms.setSelected(getChecked());
		chckbxConsolidatePrograms.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				// setChecked();
				prefs.putBoolean(VALIDATE_STRINGS, chckbxConsolidatePrograms.isSelected());
				updateTable(false);
				secretLabel.setText("  ");
				secretLabel.setText("");
			}
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (date == null) {
					...
					if (date2 == null) {
						...
						if (getChecked()) {
							try {
								...
								DataHandling.validateData(DataHandling.acceptDateTable(formattedString));
								loadTable(DataHandling.validateData(DataHandling.acceptDateTable(formattedString)));
							} catch (IOException e) {
								e.printStackTrace();
							}
					...
					if (date2 != null) {
						...
						List<String> dates = DataHandling.dateDiff(formattedString, formattedString2);
						...
						for (Map<String, Long> map : maps) {
							for (Map.Entry<String, Long> entry : map.entrySet()) {
								...
								combinedMaps.put(key, current == null ? entry.getValue() : entry.getValue() + current);
							}
	...
	/**
	 * This method stops the program and changes the status label to being stopped.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Start the program timer. This method creates a new thread from a ProgramTimer
	 * object and starts tracking. We must use a new thread to not 'hang' the GUI.
	 */
	...
	/**
	 * This method updates the table found in the main window of the gui. It works
	 * by destroying the current table object and creates a new one.
	 */
	...
	public static void updateTable(boolean fresh) {
		...
		if (loadedCurrentMap.size() == 0) {
			...
			model = new DefaultTableModel(getRows((TimeConvert.convertTime(loadedCurrentMap))), columns) {
				@Override
		...
		if (PreferencesGui.getDisplayIndex() == 3) {
			...
			Map<String, String> finalMap2 = TimeConvert.convertWritten(loadedCurrentMap);

			// Append a new column with copied data
			model.addColumn(TimeConvert.getUnit(), finalMap2.values().toArray());
		} else {
	...
	/**
	 * Setter to set the frame location variables to be used to create new windows
	 * that match the position of the main gui.
	 */
	...
	public static void loadTable(Map<String, Long> loadedMap) {
		...
		if (loadedMap.isEmpty()) {
			JOptionPane.showMessageDialog(null, "Warning: Loaded time table was empty");
		}
		...
		if (loadedMap.size() == 0) {
			...
			model = new DefaultTableModel(getRows((TimeConvert.convertTime(loadedMap))), columns) {
				@Override
				public Class getColumnClass(int column) {
					Class returnValue;
					if ((column >= 0) && (column < getColumnCount())) {
		...
		if (PreferencesGui.getDisplayIndex() == 3) {
			...
			Map<String, String> finalMap2 = TimeConvert.convertWritten(loadedMap);
	...
	/**
	 * Set the Idle check box from its saved preference, if never saved then set to
	 * false by default.
	 */
